...
With an understanding of the baseline performance variation on each system and application tested, we then combine application performance data from the sources described in Section \ref{sec:methods} to better understand how performance is affected by extrinsic factors.
...
To concisely display this information and identify metrics that most likely contribute to abnormal performance, we propose a unified measurements and metrics interface (UMAMI) diagram as demonstrated in Figure \ref{fig:umami-scratch2-hacc-write}.
...
Overlaying this weather on the climate (dashed lines in the box plots) shows how each metric compares to the distribution of past weather conditions, enabling rapid, holistic differentiation of rare events from long-term performance problems.
To demonstrate this, we use UMAMI diagrams to identify different factors that do and do not contribute to I/O performance loss in the remainder of this section.
...
As such, these analyses can be easily performed and delivered to users of production HPC systems in a straightforward fashion.
...
Thus, HACC job's poor performance was a result of I/O loads extrinsic to this job which competed for both bandwidth and metadata operation rates; the number of concurrently running jobs and HACC's placement on the dragonfly network had no clear impact.
...
Figure \ref{fig:umami-scratch3-hacc-write-long-term} shows the UMAMI view of Edison's scratch3 file system during a time when coverage factors were not unusual despite an ongoing $2\times$ slowdown over the normal 50 GiB/sec between February 24 and March 9.
