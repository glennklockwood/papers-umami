...
platforms used to conduct this work.  In section \ref{sec:results} we summarize the
...
file system logs.  In section \ref{sec:discussion} we then explain \emph{why} the
features described in section \ref{sec:discussion} arose by correlating and
...
performance variability is not an intrinsic property of a file system} and
that different I/O patterns result in different levels of variability.  Furthermore,
...
configurations (see Section \ref{sec:platforms}), their specific user workload
distributions, or a combination of both.

\begin{figure}[t]
\centering
\includegraphics[width=1.0\columnwidth]{figs/perf-boxplots-fs.pdf}
...
\subsection{Correlating Application and Server-side Telemetry} \label{sec:results/correlations}
...
Figure \ref{fig:cdfs}a and shows that the majority of tests
...
bandwidth (Figure \ref{fig:cdfs}b) is far more broadly distributed, and the
...
This is a clear indication that the coverage factor represents only one of the
factors that contribute to sub-optimal peformance, and this finding is consistent with
...
peformance relative to the maximum throughput observed across each file system
