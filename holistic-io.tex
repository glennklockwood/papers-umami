...
To make more concrete statements about the relationship between performance, coverage factor, IOPS, and other factors, we then directly correlate the data from application-level profiling and server-side monitoring as shown in Figure~\ref{fig:scatter-mira-ops}.  As mentioned in the previous section, server-side IOPS load also correlates with application-level performance, but the relationship is less clear than with the coverage factor.  In particular, Mira showed modest negative correlation between performance and the number of read ops ($r_{pearson} = -0.371$), but no significant correlation between performance and write ops.  This lack of correlation is in large part due to several tests than ran during a single period of intense write operation activity; despite the extremely high write ops registered by ggiostat, the benchmarks all ran within 80\% of peak performance.  For comparison, if the results of this one day are removed, modest correlation is found between performance and both read and write ops ($r_{pearson} = -0.368$ and $-0.320$, respectively).

% excluding job #1039807
%Write Ops -0.320345445989 0.00185127360885
%Read Ops -0.368378583027 0.000302695362085

% including job #1039807
%Write Ops  0.04480585901 0.658020505864
%Read Ops -0.370709343374 0.000146638888281

\todo{this paragraph needs to be rewritten}  However, the
...
\begin{figure}[t]
	\centering
	\includegraphics[width=\columnwidth]{figs/scatter-mira-ops.pdf}
	\caption{Fraction of peak performance, normalized to each I/O motif, along with the fraction of maximum op counts measured on mira-fs1.  Correlation coefficient between performance and read op rate is -0.371 with a p-value $\mathcal{O}(10^{-4})$; no significant correlation exists between performance and write op rate.  Linear regression line included as a visual aid.
    }
	\label{fig:scatter-mira-ops}
\end{figure}
