...
This coverage factor is a direct reflection of how much I/O traffic a job
competed against in the underlying file systems.  When $CF = 1.0$,
...
under two circumstances: (1) if server-side monitoring (e.g., LMT or ggiostat)
...
data).  To eliminate severely misaligned or incomplete data, we discard all test results where $CF > 1.2$ from the analysis that follows.
...
This is a clear indication that the coverage factor, and therefore server-side
...
\emph{and} I/O operation (IOP) rate.  When we combine the CF CDF with the CDF
of peak IOPS observed on Mira, ...
