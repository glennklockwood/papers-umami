...
\begin{document}
\title{Towards Total Knowledge of I/O through Multicomponent Instrumentation}
...
This metric provides a direct indication of if a job had exclusive access to the
file systems' bandwidth while it was running; a coverage factor of 1.0 indicates
that all of the server-side bytes were caused by the job that generated the
Darshan log, while a converage factor of 0.5 indicates that only half of the
server-side traffic was attributable to the job.

Figure \ref{fig:cdf-coverage} demonstrates the distribution of coverage
factors across all experiments run.  This distribution indicates that the
majority of tests ($> 85\%$) have high coverage factor ($ CF > 0.90 $), which
is consistent with the observation that I/O occurs in bursts\cite{Liu2016}\todo{more}, and the
...
distribution function of performance relative to the peak observed bandwidth
(Figure \ref{fig:cdf-performance}) is far more evenly distributed; this is a
clear indication that server-side interference (of which the coverage factor is
a direct indicator) is far from the only contributor to sub-optimal peformance
on all file systems tested.
...
\label{fig:coverage-factor-distribution}
\end{figure}
