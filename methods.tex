...
Storage system traffic monitoring provides insight into the aggregate systemwide I/O workload through metrics such as bytes read/written and operation counts for reads, writes, and metadata.
The data are collected in time-series format with minimal impact on application performance because the information is gathered on the storage servers, not compute nodes.
On both Mira and Edison, these data were collected at five-second intervals to balance low performance impact and sufficient granularity using file-system-specific tools.  
