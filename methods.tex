...
IBM Spectrum Scale (GPFS) file systems.  \assign{Zach}  It uses the
\texttt{mmpmon} monitoring system built into GPFS to retrieve metrics from server and
client clusters.  These metrics are queried on five second intervals by a
persistent monitoring daemon and stored in a database.
The metrics collected include bytes read and written,
read and write request counts, open and close request counts, and the
number of readdir requests. \todo{(I suggest we leave out inode updates as we don't understand what that entirely means)}. 
...
On GPFS, we record the fullness of each disk LUN that contains data and the failure status of each LUN at the start of each job. The NSD server to NSD mapping
is also collected on the login node. This mapping can identify if a server
has failed and NSDs are being handled by a secondary server. This data is
collected at the start of the job submission, so there is the queue delay
between collection and job start. This data can then be associated with a
