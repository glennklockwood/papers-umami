...
%
Job layouts on the Edison system were recorded for each job by combining queries to the Slurm accounting database, which maps jobs to node lists, and the Cray service database, which maps nodes to positions within the dragonfly network.  Job layouts on Mira are always close-packed due to its scheduling policy.
