...
Storage system traffic data such as that provided by LMT and ggiostat are also reduced into scalar values (for example, as minimum, maximum, average bytes read per second).
...
The instrumentation methods described in Section \ref{sec:methods}
do not provide a fixed reference point on I/O behavior; the workload and state of the
system naturally evolve over time.  We must therefore combine TOKIO with
routine, formalized I/O performance regression tests in order establish baseline behavior, and we developed the TOKIO Automated Benchmark Collection to meet this need.
...
%is a highly scalable code developed to 
